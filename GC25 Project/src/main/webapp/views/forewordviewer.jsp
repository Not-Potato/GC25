		...
		<jsp:include page="./common/header.jsp"></jsp:include>
 				...
 				<div class="container my-5">
	    			<div class="bg-light p-5 rounded">
						<h3>${forewordBoardDTO.getTitle() }</h3>
						<div>${forewordBoardDTO.getContents()}</div>
						
						<div>게시글 번호: ${forewordBoardDTO.getBoardNumber()}</div>
						<div>회원: ${forewordBoardDTO.getMemberNumber()}</div>
						<div>학원 명: ${forewordBoardDTO.getAcademyName()}</div>
						<div>course: ${forewordBoardDTO.getCourse()}</div>
						<div>작성날짜: ${forewordBoardDTO.getWriteDate()}</div>
						<div>recommend: ${forewordBoardDTO.getRecommend() } </div>
						<div>views:  ${forewordBoardDTO.getViews() } </div>
	     ...
	     <hr>
	    		...
	    		<div class="recommend"> 
	    			...
	    			<div id=likeCount>좋아요 : ${forewordBoardDTO.getRecommend()}</div>
	    			
	    			<form id="likeForm" action="${contextPath}/forewordboard/recommend.do?boardNum=${forewordBoardDTO.getBoardNumber()}" method="post">
